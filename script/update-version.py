#!/usr/bin/env python
#
# Copyright 2026-2026 by Martin Moene
#
# Distributed under the Boost Software License, Version 1.0.
# (See accompbiting file LICENSE.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
#
...
#        , r'set\W+phys_units_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( phys_units_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\phys_units_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'phys_units_version = "{major}.{minor}.{patch}"\n' )
...
#        , 'version = "{major}.{minor}.{patch}"' )
    ...
    , ( 'phys/units/quantity.hpp'
        , r'\#define\s+phys_units_MAJOR\s+[0-9]+\s*$'
        , '#define phys_units_MAJOR  {major}' )

    , ( 'phys/units/quantity.hpp'
        , r'\#define\s+phys_units_MINOR\s+[0-9]+\s*$'
        , '#define phys_units_MINOR  {minor}' )

    , ( 'phys/units/quantity.hpp'
        , r'\#define\s+phys_units_PATCH\s+[0-9]+\s*$'
        , '#define phys_units_PATCH  {patch}\n' )
)
